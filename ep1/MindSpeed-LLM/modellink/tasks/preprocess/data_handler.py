...
def _get_data_format(files):
    ...
    format_num = {}
    # Prefer HF-style shard files (data-*.arrow/csv/...) when present so that
    # metadata files such as dataset_info.json or state.json do not skew the
    # detected format for small splits (e.g., val only has a single arrow file).
    def _iter_files():
        data_shards = [f for f in files if os.path.basename(f).startswith('data-')]
        return data_shards if data_shards else files
