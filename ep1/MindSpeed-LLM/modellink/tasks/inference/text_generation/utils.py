...
def _post_process(batch_token_iterator, context_length, context_lengths):
    args = get_args()
    tokenizer = get_tokenizer()
    debug_batch = getattr(args, "debug_print_eval_batch", False)
    for tokens, _, log_probs in batch_token_iterator:
        context_length += 1
        if tokens is not None:
            if debug_batch and tokenizer is not None:
                try:
                    latest_tokens = tokens[:, context_length - 1].detach().cpu().tolist()
                    decoded = [tokenizer.decode([tok]) for tok in latest_tokens]
                    logger.info("[DEBUG] step=%s token_ids=%s texts=%s started_flags=%s",
                                context_length - 1,
                                latest_tokens,
                                decoded,
                                (context_lengths <= (context_length - 1)).cpu().numpy().tolist())
                except Exception as log_err:
                    logger.warning("[DEBUG] unable to decode tokens: %s", log_err)
    ...
    if parallel_state.is_pipeline_last_stage():
        ...
        if stop_ids:
            for stop_id in stop_ids:
                match = (prev == stop_id)
                if match.any():
                    logger.info("[DEBUG] matched stop_id %s started=%s", stop_id, started.cpu().numpy().tolist())
                done_token = match.byte() & started.byte()
                if done_token.any():
                    logger.info("[DEBUG] stop_id %s triggered for samples %s", stop_id, done_token.nonzero())
                is_done = is_done | done_token
...
def _sample_and_synchronize(args, batch_size, context_length_info, logits, tokens):
    debug_tokens = getattr(args, "debug_print_eval_batch", False)
    log_probs = None
    prev = None
    started = None
    ...
    if parallel_state.is_pipeline_last_stage():
        ...
        if debug_tokens:
            try:
                logger.info("[DEBUG] step=%s selected token_ids=%s",
                            context_length,
                            new_tokens.detach().cpu().tolist())
            except Exception as log_err:
                logger.warning("[DEBUG] unable to log token ids: %s", log_err)
        if parallel_state.get_pipeline_model_parallel_world_size() > 1:
