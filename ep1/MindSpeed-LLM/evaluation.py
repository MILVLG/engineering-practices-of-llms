...
def _apply_ascend_visible_devices():
    visible = os.getenv("ASCEND_VISIBLE_DEVICES")
    if not visible:
        return
    try:
        device_ids = [int(x.strip()) for x in visible.split(',') if x.strip()]  # e.g. "1,2,3"
    except ValueError:
        logger.warning("ASCEND_VISIBLE_DEVICES contains non-numeric entries: %s", visible)
        return
    if not device_ids:
        return
    try:
        ...
        if not hasattr(torch, "npu"):
            return
        original_device_count = torch.npu.device_count
        original_set_device = torch.npu.set_device
        original_current_device = torch.npu.current_device
        original_get_device_name = getattr(torch.npu, "get_device_name", None)

        logical_to_physical = {idx: dev for idx, dev in enumerate(device_ids)}
        physical_to_logical = {dev: idx for idx, dev in enumerate(device_ids)}
        ...
        def _normalize_index(index):
            ...
            if isinstance(index, int):
                if index in logical_to_physical:
                    return logical_to_physical[index]
                return index
            return index

        def _filtered_device_count():
            return len(device_ids)

        def _filtered_set_device(index):
            return original_set_device(_normalize_index(index))

        def _filtered_current_device():
            physical = original_current_device()
            return physical_to_logical.get(physical, physical)

        torch.npu.device_count = _filtered_device_count  # type: ignore[attr-defined]
        torch.npu.set_device = _filtered_set_device  # type: ignore[attr-defined]
        torch.npu.current_device = _filtered_current_device  # type: ignore[attr-defined]

        os.environ.setdefault("ASCEND_DEVICE_ID", str(device_ids[0]))
        try:
            torch.npu.set_device(0)  # logical index 0
        except Exception as set_err:
            logger.warning("Failed to set logical NPU device 0 -> %s: %s", device_ids[0], set_err)
        logger.info("[DEBUG] ASCEND_VISIBLE_DEVICES=%s, original device_count=%s, filtered=%s",
                    visible, original_device_count(), len(device_ids))
    except Exception as err:  # broad catch so evaluation仍可继续
        logger.warning("Unable to honor ASCEND_VISIBLE_DEVICES=%s: %s", visible, err)


# _apply_ascend_visible_devices()
...
class LLMChat(Chat):
    ...
    def chat(self, instruction, history):
        ...
        # debug_batch = getattr(self.args, "debug_print_eval_batch", False)
        # if debug_batch:
        #     logger.info("[DEBUG] LLMChat.chat() batch=%s instructions-preview=%s",
        #                 len(instruction),
        #                 [ins[:80] + ("..." if len(ins) > 80 else "") for ins in instruction[:2]])
...
def main():
    initialize_megatron(extra_args_provider=add_text_generate_args,
                        args_defaults={'no_load_rng': True,
                                       'no_load_optim': True})
    args = get_args()
    # debug_env = os.getenv("EVAL_DEBUG_BATCH", "0")
    # if debug_env == "1":
    #     setattr(args, "debug_print_eval_batch", True)
    #     print_rank_0(f"[DEBUG] evaluation batch size arg: {args.evaluation_batch_size}")

    if args.add_eos_token:
        setattr(args, "add_eos_token", args.add_eos_token)
