...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # === 核心：用当前输入的 device，而不是 self.device ===
        ...
        # === 安全计算 kv_seq_len，避免 past_key_value 为 None 时报错 ===
        ...
        # 这个是当前 step 的 K/V，用来构造下一步的 cache
        output_cache = (key_states, value_states)

        # === 如果有 past_key_value，则从大 cache 里 slice 出本层的历史 K/V 并拼接 ===
        if past_key_value is not None:
        ...
        # 这里你已经把 attention_mask 预处理成 [bsz, 1, q_len, kv_len] 的大负数 mask
