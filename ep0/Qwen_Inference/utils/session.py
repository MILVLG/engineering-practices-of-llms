...
class PyTorchSession(Session):
    def __init__(self, config: InferenceConfig) -> None:
        ...
        if hasattr(config, "device_str"):
            self.device_str = config.device_str
        else:
            self.device_str = "npu"

        self.device = torch.device(self.device_str)
        print(f"[INFO] PyTorchSession device = {self.device}")

        # ✅ 正常情况下在这里就创建好 kv_cache
        self.kv_cache = create_kv_cache(config)
    ...
    def _to_device(self, x):
        """把 numpy / tensor 统一搬到 self.device（保险起见）。"""
        if x is None:
            return None
    ...
    def run(self, input_ids, show_progress: bool = False):
        # 🔥 兜底：如果是旧实例（当年 __init__ 没有 kv_cache），这里即时创建一个
        if not hasattr(self, "kv_cache"):
            print("[WARN] PyTorchSession has no kv_cache, creating one on the fly")
            self.kv_cache = create_kv_cache(self.config)

        # 1) input_ids → NPU long tensor
        if isinstance(input_ids, np.ndarray):
            input_ids = torch.from_numpy(input_ids)
        input_ids = input_ids.to(self.device).long()
        seq_len = input_ids.shape[-1]

        # 2) 用 KV cache 生成 cache / mask / pos_ids
        cache, mask, pos_ids = self.kv_cache.get_inputs(seq_len)

        # 保险：确保都在 NPU 上
        cache = self._to_device(cache)
        mask = self._to_device(mask)
        pos_ids = self._to_device(pos_ids)
        ...
        # 4) 更新 KV cache
        if hasattr(self.kv_cache, "update"):
            self.kv_cache.update(seq_len, result[1])

        # 5) 返回 logits
        logits = result[0].detach().cpu().numpy()
        return logits
